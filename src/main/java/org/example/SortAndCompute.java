...
public class SortAndCompute {
    ...
    static  double Sort_Compute(String content1,String content2,String[] keyWordSet){
        ...
        for (int i=0;i<keyWordSet.length;i++){
            //获得关键词所在位置下标的链表
            IndexList list= IndexOfKeyword.Compute_kmp(charContent1,keyWordSet[i].toCharArray());
        ...
        for (int i=0;i<keyWordSet.length;i++){
            IndexList list= IndexOfKeyword.Compute_kmp(charContent2,keyWordSet[i].toCharArray());
            Node node= list.getHead();
